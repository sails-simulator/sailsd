...
static void draw_grid(cairo_t *cr) {
    ...
    int n;
    for (n = -SAIL_GRID_N; n <= SAIL_GRID_N; n++) {
...
static void draw_axis(cairo_t *cr) {
    ...
    char number_text[SAIL_GRID_NUMBER_STRING_LEN];
    int n;
    for (n = -SAIL_GRID_WIDTH; n <= SAIL_GRID_WIDTH; n += SAIL_GRID_SPACING) {
        cairo_move_to(cr, n + 10, -10);
        snprintf(number_text, SAIL_GRID_NUMBER_STRING_LEN, "%d", n/SAIL_GRID_SPACING);
